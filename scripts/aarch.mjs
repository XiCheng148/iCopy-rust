...
async function resolve() {
  ...
  const appPathList = [
    join("macos/iCopy.aarch64.app.tar.gz"),
    join("macos/iCopy.aarch64.app.tar.gz.sig"),
  ];
  ...
  fs.copyFileSync(join("macos/iCopy.app.tar.gz"), appPathList[0]);
  fs.copyFileSync(join("macos/iCopy.app.tar.gz.sig"), appPathList[1]);

  const options = { owner: "XiCheng148", repo: "iCopy" };
  ...
  await uploadAssets(release.id, [
    join(`dmg/iCopy_${version}_aarch64.dmg`),
    ...appPathList,
  ]);
}
